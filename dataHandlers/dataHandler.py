...
class Raw2RawDataset:
    def __init__(self, root_dir, num_test_people, num_val_people, random_seed=42, segment_length=10000, NFFT=300, tfr_len=64):
        self.NFFT = NFFT
        self.segment_length = segment_length
        self.tfr_len = tfr_len
        self.random_state = np.random.RandomState(random_seed)
        self.num_test_people = num_test_people
        self.num_val_people = num_val_people
        ...
        self.test_people = self.random_state.choice(self.all_people, size=self.num_test_people+self.num_val_people, replace=False)
        self.train_people = self.all_people[~np.isin(self.all_people, self.test_people)]
        
        self.val_people = self.random_state.choice(self.test_people, size=self.num_val_people, replace=False)
        self.test_people = self.test_people[~np.isin(self.test_people, self.val_people)]
    ...
    def get_sample(self, people):
        ...
        start = self.random_state.randint(540, min(full_eeg.shape[-1], 300 * 30 * 72 // 4)-self.segment_length)
    ...
    def get_val_batch(self, n):
        return self.get_batch(n, self.val_people)
